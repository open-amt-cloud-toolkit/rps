...
import { AMTConfigurations, NETConfigurations } from "./models/Rcs";
...
export class NetConfigFileStorageDb implements INetProfilesDb {
    networkConfigs: NETConfigurations;
    amtProfiles: AMTConfigurations;
    private logger: ILogger;

    constructor(amtProfiles: AMTConfigurations, networkConfigs: NETConfigurations, logger: ILogger) {
        this.logger = logger;
        this.logger.debug(`using local NetConfigFileStorageDb`);
        this.networkConfigs = networkConfigs || (new NETConfigurations());
    ...
    async deleteProfileByName(netConfigName: any): Promise<any> {
        ...
        for (var i = 0; i < this.networkConfigs.length; i++) {
            this.logger.silly(`Checking ${this.networkConfigs[i].ProfileName}`)
            if (this.networkConfigs[i].ProfileName === netConfigName) {
              let profileUsingThisConfig = this.amtProfiles.find(profile => profile.NetworkConfigName === netConfigName)
    ...
    async updateProfile(netConfig: NetworkConfig): Promise<any> {
        ...
        var isMatch = item => item.ProfileName === netConfig.ProfileName;
        ...
        var index = this.networkConfigs.findIndex(isMatch);
