...
import { DOMAIN_INSERTION_SUCCESS, DOMAIN_ERROR, DOMAIN_SUCCESSFULLY_UPDATED, DOMAIN_NOT_FOUND } from "../../../utils/constants";

export async function editDomain(req, res) {
  let domainsDb: IDomainsDb = null;
  let log = Logger("editDomain")
  ...
  try {
    ...
    const results = await domainsDb.updateDomain(amtDomain).catch((reason) => { errorReason = reason })
