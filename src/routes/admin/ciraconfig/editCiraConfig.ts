...
import { CIRA_CONFIG_INSERTION_SUCCESS, CIRA_CONFIG_ERROR, PROFILE_INVALID_INPUT, CIRA_CONFIG_UPDATE_SUCCESS, CIRA_CONFIG_NOT_FOUND } from "../../../utils/constants";

export async function editCiraConfig (req, res) {
  let log = Logger(`editCiraConfig`);
  ...
  try {
    ...
    let pwdBefore = ciraConfig.Password;
    ...
    const results = await ciraConfigDb.updateCiraConfig(ciraConfig).catch((reason) => { errorReason = reason })
...
function readBody(req, res): CIRAConfig {
  let config: CIRAConfig = <CIRAConfig>{};
  let body = req.body;
