...
export async function createCiraConfig (req, res) {
  let log = Logger(`createCiraConfig`);
  ...
  try {
    ...
    const results = await ciraConfigDb.insertCiraConfig(ciraConfig).catch((reason) => { errorReason = reason })
    ...
    if(!errorReason && results)
      res.status(200).end(CIRA_CONFIG_INSERTION_SUCCESS(ciraConfig.ConfigName))
    else {
