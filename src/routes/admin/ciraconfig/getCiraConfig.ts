...
export async function getCiraConfig (req, res): Promise<void> {
  ...
  try {
    ...
    if (results != null) {
      ...
      Logger.info(`CIRA config profile : ${JSON.stringify(results)}`)
      res.status(200).json(API_RESPONSE(results)).end()
    } else {
      Logger.info(`Not found : ${ciraConfigName}`)
      res.status(404).json(API_RESPONSE(null, 'Not Found', CIRA_CONFIG_NOT_FOUND(ciraConfigName))).end()
    }
  } catch (error) {
    Logger.error(`Failed to get CIRA config profile : ${ciraConfigName}`, error)
