...
describe('networkConfigurator Tests', () => {
  beforeAll(() => {
    clientId = uuid()
    activationmsg = {
      method: 'activation',
  ...
  describe('validate Ethernet Port Settings', () => {
    ...
    test('should return null if detected ethernet is wireless', async () => {
      const message = {
        Envelope: {
          Header: {
            Action: {
    ...
    test('should return wsman if PhysicalConnectionType is not 3', async () => {
      const message = {
        Envelope: {
          Header: {
            Action: {
    ...
    test('should return wsman if PhysicalConnectionType not present', async () => {
      const message = {
        Envelope: {
          Header: {
            Action: {
    ...
    test('should handle fault returned from setEthernetPortSettings Put message', async () => {
      const message = {
        Envelope: {
          Header: {
            Action: {
              _: 'http://schemas.dmtf.org/wbem/wsman/1/wsman/fault'
            },
            ResourceURI: 'http://schemas.xmlsoap.org/ws/2004/08/addressing/role/anonymous'
          },
          Body: {
            Fault: {
              Code: {
                Value: 'a:Sender',
                Subcode: {
                  Value: 'e:UnsupportedFeature'
                }
              },
              Reason: {
                Text: 'The specified feature is not supported.'
              },
              Detail: {}
            }
          }
    ...
    test('should return null if ethernet LinkIsUp is false', async () => {
      const message = {
        Envelope: {
          Header: {
            Action: {
