...
export class CommandParser {
    ...
    static logger: ILogger = Logger(`CommandParser`);

    static parse(msg: ClientMsg): ClientMsg {
        try {

            if (msg && msg.method && msg.method.length > 0) {
                let input: string[] = msg.method.trim().split(' ');

                this.logger.debug(`split input: ${input}`);

                let args = parseArgs(input, options);
        ...
        } catch (error) { this.logger.error(`parse exception: ${error}`) }
