...
export class CertManager implements ICertManager {
   ...
   */
    dumpPfx(pfxobj: any): any {
        let provisioningCertificateObj = new ProvisioningCertObj();
        let interObj = new Array<certificateObject>();
        let leaf = new certificateObject();
        let root = new certificateObject();
        if (pfxobj) {
            let fingerprint: string;
            if (pfxobj.certs && Array.isArray(pfxobj.certs)) {
                for (let i = 0; i < pfxobj.certs.length; i++) {
                    let cert: any = pfxobj.certs[i];
                    ...
                    if (i == 0) {
                        ...
                        let der: string = this.nodeForge.asn1ToDer(this.nodeForge.pkiCertificateToAsn1(cert)).getBytes();
                        let md: any = this.nodeForge.sha256Create();

                        md.update(der);
                        fingerprint = md.digest().toHex();
                    }
                    else {
                        let inter = new certificateObject();
                        inter.pem = pem;
    ...
    convertPfxToObject(pfxb64: string, passphrase: string): any {
        let pfx_out: any = { certs: [], keys: [] };

        let pfxder: string = this.nodeForge.decode64(pfxb64);
        let asn: any = this.nodeForge.asn1FromDer(pfxder);
        let pfx: any;
        try {
        ...
        for (let i = 0; i < bags[this.nodeForge.pkiOidsCertBag].length; i++) {
            // dump cert into DER
            let cert: any = bags[this.nodeForge.pkiOidsCertBag][i];
        ...
        for (let i = 0; i < bags[this.nodeForge.pkcs8ShroudedKeyBag].length; i++) {
            // dump cert into DER
            let cert: any = bags[this.nodeForge.pkcs8ShroudedKeyBag][i];
