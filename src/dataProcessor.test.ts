...
describe('process client data', () => {
  test('Should return an error with activation message and junk payload', async () => {
    ...
    const clientId = uuid()
    clientManager.addClient({ ClientId: clientId, ClientSocket: null, messageId: 0 })
...
describe('deactivate Device', () => {
  it('should return client message with deactivation WSMan as payload', async () => {
    ...
    const clientId = uuid()
    clientManager.addClient({ ClientId: clientId, ClientSocket: null, messageId: 0, connectionParams: connectionParams })
  ...
  it('should return client message with deactivation WSMan as payload on 401 response', async () => {
    ...
    const clientId = uuid()
    clientManager.addClient({ ClientId: clientId, ClientSocket: null, ClientData: clientData, messageId: 0, connectionParams: connectionParams })
    VersionChecker.setCurrentVersion('4.0.0')
  ...
  it('should return client message with deactivation WSMan as payload on 401 response', async () => {
    ...
    const clientId = uuid()
    clientManager.addClient({ ClientId: clientId, ClientSocket: null, ClientData: clientData, messageId: 0 })
    VersionChecker.setCurrentVersion('4.0.0')
    const responseMsg = await dataProcessor.handleResponse(clientMsg, clientId)
    expect(buildResponseSpy).toHaveBeenCalled()
