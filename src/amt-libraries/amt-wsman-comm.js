...
var CreateWsmanComm = function (host, port, user, pass, tls, tlsoptions, setupCommunication) {
    ...
    function SerialTunnel(options) {
        ...
        obj._write = function (chunk, encoding, callback) { 
            ...
            } if (callback) callback(); }; // Pass data written to forward
    ...
    obj.xxConnectHttpSocket = function () {
        ...
        if (obj.setupCommunication != null) {
            if (obj.xtls != 1) {
              ...
              obj.socket.onStateChange = function (ccon, state) {
                ...
                try {
                  obj.cirasocket = obj.setupCommunication(obj.host, obj.port);
                  if (obj.cirasocket == null) { obj.xxOnSocketClosed(); return; }
                    ...
                    obj.cirasocket.onStateChange = function (ciraconn, state) {
                        ...
                        if (state == 2) {
                            ...
                            try{
                                obj.socket = obj.tls.connect(obj.port, 'localhost', options, function() { obj.xxOnSocketConnected() });
                            } catch(error) {
                                console.log(error)
                            }
                            ...
                            obj.socket.on('close', function(ex) {
                                console.log(ex)
                                obj.xxOnSocketClosed();
                            });
                            obj.socket.on('timeout', function() {
                                obj.destroy()
                            });
                            ...
                            obj.socket.on('data', function (data) {
                                console.log(data)
                                try { 
                                    obj.xxOnSocketData(data.toString('binary')); 
                                    } catch (e) { 
    ...
    obj.destroy = function () {
        if (obj.socket != null) {
            ...
            try {
              if (obj.cirasocket != null) { 
                  obj.cirasocket.close(); 
              } else { 
                  obj.socket.close();
              }
            } catch (ex) { }
    ...
    obj.xxOnSocketClosed = function (data) {
        ...
        if (obj.pendingAjaxCall.length > 0) {
            ...
            setTimeout(function () {
                ...
                }, 500); // Wait half a second and try again
        }
    }
