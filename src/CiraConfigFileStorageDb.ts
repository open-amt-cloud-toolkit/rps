import { RCSConfig, CIRAConfigurations, AMTConfigurations } from "./models/Rcs";
...
export class CiraConfigFileStorageDb implements ICiraConfigDb {
    ...
    constructor(amtProfiles: AMTConfigurations, ciraConfigs: CIRAConfigurations, logger: ILogger) {
        this.logger = logger;
    ...
    async deleteCiraConfigByName(ciraConfigName: any): Promise<any> {
        ...
        for (var i = 0; i < this.ciraConfigs.length; i++) {
            this.logger.silly(`Checking ${this.ciraConfigs[i].ConfigName}`)
            if (this.ciraConfigs[i].ConfigName === ciraConfigName) {
              let profileUsingThisConfig = this.amtProfiles.find(profile => profile.CIRAConfigName === ciraConfigName)
