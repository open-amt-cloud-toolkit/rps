...
export class Deactivation {
  ...
  removeDeviceFromSecretProvider = async ({ input }: { input: DeactivationContext }): Promise<boolean> => await this.configurator.secretsManager.deleteSecretAtPath(`devices/${devices[input.clientId].uuid}`)

  removeDeviceFromMPS = async ({ input }: { input: DeactivationContext }): Promise<any> => await got(`${Environment.Config.mps_server}/api/v1/devices/${devices[input.clientId].uuid}?tenantId=${input.tenantId}`, {
    method: 'DELETE'
  })
