    ...
    createMachine({
      ...
      states: {
        ...
        GET_CIRA_CONFIG: {
          invoke: {
            ...
            src: fromPromise(async ({ input }) => {
              const data = await this.configurator.profileManager.getCiraConfiguration(input.profileName, input.tenantId)
              return data
            }),
            input: ({ context }) => ({
              userId: context.profile.profileName,
        ...
        SET_MPS_PASSWORD: {
          invoke: {
            src: fromPromise(async ({ input }) => {
              const data = await this.configurator.secretsManager.getSecretAtPath(`CIRAConfigs/${input.configName}`)
              return data
            }),
            input: ({ context }) => ({
              configName: context.ciraConfig.configName
            }),
            id: 'set-mps-password',
            onDone: {
      ...
      guards: {
        userInitiatedConnectionServiceSuccessful: ({ event }) => event.data.Envelope.Body?.RequestStateChange_OUTPUT?.ReturnValue === 0,
