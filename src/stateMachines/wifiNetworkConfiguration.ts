...
export class WiFiConfiguration {
  ...
  getWifiProfile = async ({ input }: { input: WiFiConfigContext }): Promise<void> => {
    if (input.amtProfile?.wifiConfigs != null) {
      ...
      input.wifiProfile = await this.db.wirelessProfiles.getByName(input.amtProfile.wifiConfigs[input.wifiProfileCount].profileName, input.amtProfile.tenantId)
      if (input.wifiProfile != null) {
        if (input.wifiProfile.ieee8021xProfileName != null) {
          input.wifiProfile.ieee8021xProfileObject = await this.db.ieee8021xProfiles.getByName(input.wifiProfile.ieee8021xProfileName, input.wifiProfile.tenantId)
          input.authProtocol = input.wifiProfile.ieee8021xProfileObject != null ? input.wifiProfile.ieee8021xProfileObject.authenticationProtocol : 0
        }
        if (this.configurator?.secretsManager) {
          // Get WiFi profile pskPassphrase from vault
          const data = await this.configurator.secretsManager.getSecretAtPath(`Wireless/${input.wifiProfile.profileName}`) as WifiCredentials
          if (data != null) {
  ...
  addWifiConfigs = async ({ input }: { input: WiFiConfigContext }): Promise<any> => {
    ...
    const wifiEndpointSettings: CIM.Models.WiFiEndpointSettings = {
      ...
      InstanceID: input.wifiProfile != null ? `Intel(r) AMT:WiFi Endpoint Settings ${input.wifiProfile.profileName}` : '',
      AuthenticationMethod: input.wifiProfile != null ? input.wifiProfile.authenticationMethod as CIM.Types.WiFiEndpointSettings.AuthenticationMethod : 6,
      EncryptionMethod: input.wifiProfile != null ? input.wifiProfile.encryptionMethod as CIM.Types.WiFiEndpointSettings.EncryptionMethod : 4,
      SSID: input.wifiProfile != null ? input.wifiProfile.ssid : '',
      Priority: input.amtProfile?.wifiConfigs != null ? input.amtProfile.wifiConfigs[input.wifiProfileCount].priority : 0
    }
    ...
    if (wifiEndpointSettings.AuthenticationMethod === 5 || wifiEndpointSettings.AuthenticationMethod === 7) {
      ...
      const ieee8021xSettings: CIM.Models.IEEE8021xSettings = {
        ElementName: input.wifiProfile?.ieee8021xProfileObject != null ? input.wifiProfile.ieee8021xProfileObject.profileName : '',
        InstanceID: input.wifiProfile?.ieee8021xProfileObject != null ? `Intel(r) AMT: 8021X Settings ${input.wifiProfile.ieee8021xProfileObject.profileName}` : '',
      ...
      const cert: string = input.authProtocol === 2 ? null : input.addCertResponse?.AddCertificate_OUTPUT?.CreatedCertificate?.ReferenceParameters?.SelectorSet?.Selector?._
      const root: string = input.addTrustedRootCertResponse?.AddTrustedRootCertificate_OUTPUT?.CreatedCertificate?.ReferenceParameters?.SelectorSet?.Selector?._
      input.xmlMessage = input.amt?.WiFiPortConfigurationService.AddWiFiSettings(wifiEndpointSettings, selector, ieee8021xSettings, cert, root)
    } else {
  ...
  putWifiPortConfigurationService = async ({ input }: { input: WiFiConfigContext }): Promise<any> => {
    const wifiPortConfigurationService: AMT.Models.WiFiPortConfigurationService = input.message.Envelope.Body.AMT_WiFiPortConfigurationService
  ...
  addCertificate = async ({ input }: { input: { context: WiFiConfigContext, event: WiFiConfigEvent } }): Promise<any> => {
  ...
  machine =
    setup({
      ...
      guards: {
        is8021xProfileAssociated: ({ context }) => context.wifiProfile?.ieee8021xProfileName != null && context.eaResponse == null && context.addCertResponse == null,
        isMoreWiFiProfiles: ({ context }) => context.amtProfile?.wifiConfigs != null ? context.wifiProfileCount < context.amtProfile.wifiConfigs.length : false,
        isWiFiProfilesExist: ({ context }) => context.amtProfile?.wifiConfigs != null ? context.amtProfile.wifiConfigs.length > 0 : false,
        ...
        shouldRetry: ({ context, event }) => context.retryCount != null ? context.retryCount < 3 && event.output instanceof UNEXPECTED_PARSE_ERROR : false
      },
      actions: {
      ...
      states: {
        ...
        ADD_WIFI_SETTINGS: {
          invoke: {
            ...
            onError: {
              actions: assign({
                profilesFailed: ({ context }) => context.profilesFailed == null ? `${context.wifiProfileName}` : `${context.profilesFailed}, ${context.wifiProfileName}`
              }),
              target: 'CHECK_ADD_WIFI_SETTINGS_RESPONSE'
            }
          }
        ...
        SUCCESS_SYNC_ONLY: {
          entry: [assign({ statusMessage: () => 'Wireless Only Local Profile Sync Configured' }), 'Update Configuration Status'],
          type: 'final'
        }
      }
    })
