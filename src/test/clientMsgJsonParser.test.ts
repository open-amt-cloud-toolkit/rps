...
let jsonParser = new ClientMsgJsonParser(nodeForge);
...
describe("Parse the message received from client", () => {
    ...
    test("When method is activation, it should decode payload and return the message ", () => {
        ...
        let clientMsg = jsonParser.parse(msg, clientId);
        expect(clientMsg).toEqual(activationmsg);
    });

    test("When method is activation, it does not contain the mandotary field ver ", () => {
        let msg = {
            method: "activation",
            apiKey: "key",
            appVersion: "1.0.0",
        ...
        let rpsError, clientMsg = null;
        try {
            let clientId = uuid();
            clientManager.addClient({ ClientId: clientId, ClientSocket: null });
            clientMsg = jsonParser.convertClientMsg(msg, clientId);
        } catch (error) {
            rpsError = error;
        }
    ...
    test("When method is response, payload is null", () => {
        let msg = `{\"apiKey\":\"key\",\"appVersion\":\"1.0.0\",\"message\":\"all's good!\",\"method\":\"response\",\"payload\":\"\",\"protocolVersion\":\"2.0.0\",\"status\":\"ok\"}`;
        let clientId = uuid();
        clientManager.addClient({ ClientId: clientId, ClientSocket: null });
        let rpsError, clientMsg;
        try {
            clientMsg = jsonParser.parse(msg, clientId);
        } catch (error) {
            rpsError = error;
        }
        expect(rpsError).toBeInstanceOf(RPSError);
        expect(rpsError.message).toEqual(`Missing payload`);
    });
    afterEach(() => {
