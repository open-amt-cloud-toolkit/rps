...
describe("Client Actions", () => {
  ...
  beforeEach(() => {
    clientActions = new ClientActions(Logger(`ClientActions`), configurator, certManager, helper, responseMsg, amtwsman, clientManager, validator);
    activationmsg = {
        method: "activation",
        apiKey: "key",
        appVersion: "1.0.0",
  ...
  test("client action is null or undefined", async () => {
    ...
    let rpsError, responseMsg;
    let clientId = uuid();
    clientManager.addClient({ ClientId: clientId, ClientSocket: null , ClientData: activationmsg});
    try{
        responseMsg = await clientActions.BuildResponseMessage(clientMsg, clientId);
    }catch(error){
        rpsError = error;
    }
    expect(rpsError.message).toEqual(`Failed to retrieve the client activation message`);
  });
  ...
  test("client action is invalid", async () => {
    ...
    let rpsError, responseMsg;
    let clientId = uuid();
    activationmsg.payload.action = "invalid";
    clientManager.addClient({ ClientId: clientId, ClientSocket: null , ClientData: activationmsg});
    try{
        responseMsg = await clientActions.BuildResponseMessage(clientMsg, clientId);
    }catch(error){
